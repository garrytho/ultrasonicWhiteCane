...
void setup() {
  ...
  //beginning the LED strip so we can write to it later
  strip.begin();
  strip.show();
}
  ...
  if (currentDistance < 150 && currentDistance > 0) {
    ...
    if (currentDistance >= prevDistance - 5 && currentDistance <= prevDistance + 5) {
      ...
      if (readingCount > 4) {
        ...
        //LED CONTROLS:
        //map the distance (in cm) to a LED brightness. Closer is be brighter. Using Red
        ledBrightness = map(int(currentDistance), 120, 30, 255, 0);
    ...
    //turn the motor off if we've missed too many times. Also, stop the led
    if(missCount >= 5){
